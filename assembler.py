    ...
    try:
        with open(filename, 'r') as file:
            for line in file.readlines():
                ...
                line = re.sub(r'^assembly_code = """', '', line)
                line = re.sub(r'"""$', '', line)
                line = re.sub(r'/.*', '', line)
    ...
    try:
        ...
        for line in asmlist:
            if line[0] == 'org':
                ...
                instruction = line[0] if not is_label(line[0]) else line[1]
                bit_15 = "1" if line[-1] == 'i' else "0"

                if instruction in memory:
                    address = address_symbol_table[line[1]]
...
def main():
    """Main function to orchestrate the assembly process."""
    try:
        asm_file = 'assembly.txt'  # Automatically opens this file
